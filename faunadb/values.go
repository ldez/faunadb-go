...
func (date DateV) String() string {
	return fmt.Sprintf("Date(%s)", strconv.Quote(time.Time(date).Format("2006-01-02")))
}
...
func (localTime TimeV) String() string {
	return fmt.Sprintf("Time(%s)", strconv.Quote(time.Time(localTime).Format("2006-01-02T15:04:05.999999999Z")))
}
...
func (ref RefV) String() string {
	res := []string{}
	if len(ref.ID) != 0 {
		res = append(res, "ID: \""+ref.ID+"\"")
	}
	if ref.Collection != nil {
		res = append(res, "Collection: "+ref.Collection.String())
	} else if ref.Class != nil {
		res = append(res, "Class: "+ref.Class.String())
	}

	if ref.Database != nil {
		res = append(res, "Database: "+ref.Database.String())
	}

	return fmt.Sprintf("RefV{%s}", strings.Join(res, ", "))
}
...
func (set SetRefV) String() string {
	return fmt.Sprintf("SetRefV{ %s }", ObjectV(set.Parameters).String())
}
...
func (bytes BytesV) String() string {
	s := []string{}
	for b := range bytes {
		s = append(s, string(byte(b)))
	}
	return fmt.Sprintf("BytesV{%s}", strings.Join(s, " ,"))
}
...
func (query QueryV) String() string {
	return fmt.Sprintf("QueryV{%s}", query.lambda)
}
